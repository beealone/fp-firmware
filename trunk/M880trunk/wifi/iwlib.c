...
void iw_print_stats(char *buffer, int buflen, const iwqual *qual, const iwrange *range, int has_range)
{
	...
	if(has_range && ((qual->level != 0) || (qual->updated & (IW_QUAL_DBM | IW_QUAL_RCPI))))
	{
		...
		{
			len = snprintf(buffer, buflen, "Quality%c%d/%d  ", qual->updated & IW_QUAL_QUAL_UPDATED ? '=' : ':',
		...
		if(qual->updated & IW_QUAL_RCPI)
		{
			...
			if(!(qual->updated & IW_QUAL_NOISE_INVALID))
			{
				...
				len = snprintf(buffer, buflen, "Noise level%c%g dBm", qual->updated & IW_QUAL_NOISE_UPDATED ? '=' : ':',
						rcpinoise);
			}
		}
		else
...
//		snprintf(buffer, buflen, "Quality:%d  Signal level:%d  Noise level:%d", qual->qual, qual->level, qual->noise);
		snprintf(buffer, buflen, "Quality:%d", qual->qual);
	}
}
